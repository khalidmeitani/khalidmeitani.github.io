...
const projectMotionPreviews = Array.from(document.querySelectorAll("[data-motion-preview]"));

const stopProjectMotion = (preview) => {
  preview.classList.remove("is-motion-active", "is-touch-preview");

  const video = preview.querySelector(".project-motion-video");
  if (!video) return;
...
const startProjectMotion = (preview, { touch = false } = {}) => {
  if (reduceMotion) return;

  preview.classList.add("is-motion-active");
  preview.classList.toggle("is-touch-preview", touch);
...
projectMotionPreviews.forEach((preview) => {
  let lastPointerWasTouch = false;

  preview.addEventListener("pointerenter", (event) => {
    if (event.pointerType !== "touch") startProjectMotion(preview);
  });

  preview.addEventListener("pointerleave", (event) => {
    if (event.pointerType !== "touch") stopProjectMotion(preview);
  });

  preview.addEventListener("focusin", () => startProjectMotion(preview));
  preview.addEventListener("focusout", () => stopProjectMotion(preview));
  ...
  preview.addEventListener("click", (event) => {
    const isTouchClick = lastPointerWasTouch;
    lastPointerWasTouch = false;

    if (!isTouchClick || reduceMotion) return;

    if (!preview.classList.contains("is-touch-preview")) {
      event.preventDefault();
      projectMotionPreviews.forEach((item) => {
        if (item !== preview) stopProjectMotion(item);
      });
      startProjectMotion(preview, { touch: true });
      return;
    }

    window.setTimeout(() => stopProjectMotion(preview), 0);
  });
});

document.addEventListener("pointerdown", (event) => {
  if (event.target.closest?.("[data-motion-preview]")) return;
  projectMotionPreviews.forEach(stopProjectMotion);
});

document.addEventListener("visibilitychange", () => {
  if (document.hidden) projectMotionPreviews.forEach(stopProjectMotion);
});
